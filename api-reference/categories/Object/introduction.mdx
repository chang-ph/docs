...
requests.post(
    ...
    json={"payload": {"object_type": "workflow"}}
)
```
...
requests.post(
    ...
    json={
        "payload": {
            ...
            "uri": "https://vectorly-ai.github.io/gallery/gallery/statistical_rethinking/africa.zip"
        }
    }
)
```
...
requests.post(
    f"{root}/invoke",
    headers={"Authorization": f"Bearer {token}"},
    json={"target": workflow_id, "action": "run_sample", "get_response": True}
)
```
...
```python
requests.get(
    f"{root}/api/object/{experiment_id}",
    headers={"Authorization": f"Bearer {token}"}
)
```
